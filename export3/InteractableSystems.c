...
void Interactable_resolveInteraction(Layout* layouts, int numberOfLayouts, char* currentLayoutName, Interactable* interactable, Interaction* interaction, GameState gameState, bool* activeInteractions) {
	...
	case COLLISION: {
		...
		case HEALTH_PICKUP: {
			...
			Health* health = ECS_getComponent(HEALTH, *currentLayout, data->entityID);
		...
		case ENEMY: {
			if (data->collisionNormal.y < 0 && data->collisionNormal.x == 0) {
				PhysicsBody* mario = ECS_getComponent(PHYSICS_BODY, *currentLayout, data->entityID);
				if (mario == NULL) break;
				mario->velocity.y = -1000;

				Health* goombaHealth = ECS_getComponent(HEALTH, *currentLayout, interactable->ENTITY_ID);
				if (goombaHealth == NULL) break;
				goombaHealth->value = 0;


				printf("killed goomba\n");
			}
			// TODO: decrease player's health when they come in contact with a goomba from the sides. 
			if (data->collisionNormal.y == 0 && data->collisionNormal.x != 0) {
				Health* marioHealth = ECS_getComponent(HEALTH, *currentLayout, data->entityID);
				if (!marioHealth->isInvulnerable) {
					marioHealth->value -= 1.0;
					marioHealth->timeSinceLastDamage = 0;
					marioHealth->isInvulnerable = true;
				}
	...
	case CLICK: {
		...
		case BUTTON: {
			...
			// TODO: Implement same action as in the keydown section. 
